/* File generated by DarkEdifPostBuildTool v1.0.0.7, part of DarkEdif SDK.
   ...
   Copyright of the Noise extension and all rights reserved by the creator(s) of Noise.

   A native extension needs a Java file to generate a Java class. The class is empty,
   and a native is assumed by presence of CRunNoise in the assets folder.
   This file is required. The Noise creator may modify the copyright to suit their wishes,
...
public class CRunNoise extends CRunExtension
{
	...
	public CRunNoise()
	{
		if (!inited)
		{
			String libName = "Noise";

			try
			{
				android.util.Log.v("Noise", "Loading Noise extension from \"app/main/jniLibs/" + MMFRuntime.inst.ABI + "/lib" + libName + ".so\"...");
				System.loadLibrary(libName);

				android.util.Log.v("Noise", "Loaded Noise extension OK!");

				inited = true;
			}
			catch (UnsatisfiedLinkError e)
			{
				android.util.Log.e("Noise", "Couldn't load extension Noise, library lib" + libName + ".so couldn't be loaded: " + e.toString());
			}
		}
	}
	...
	public boolean createRunObject(CBinaryFile file, CCreateObjectInfo cob, int version)
	{
		...
		if (cptr == 0)
			android.util.Log.e("Noise", "Noise's createRunObject returned NULL!");
	...
	public boolean condition(int num, CCndExtension cnd)
	{
		...
		// by checking num and the return of Noise_conditionJump, which is either a 32-bit integer,
	...
	public int darkedif_jni_getActionOrConditionIntParam(CEvent evt, int paramIndex, int paramType)
	{
		if (!(evt instanceof CCndExtension) && !(evt instanceof CActExtension))
			android.util.Log.e("Noise", "darkedif_jni_getActionOrConditionIntParam: The CEvent is not CActExtension or CCndExtension. Type is \"" + evt.getClass().getSimpleName() + "\".");
		if (paramIndex == -1)
			android.util.Log.e("Noise", "darkedif_jni_getActionOrConditionIntParam: paramIndex -1.");
		...
		if (paramType == 29)
		{
			...
			{
				android.util.Log.e("Noise", "darkedif_jni_getActionOrConditionIntParam: Passed a direction " + result + " which cannot translate into NewDirection bitmask. Using no direction.");
				return 0;
			}
			return 1 << result;
		}
		...
		if (paramType != 22)
			android.util.Log.w("Noise", "darkedif_jni_getActionOrConditionIntParam: Unknown param type " + paramType + " requested. Defaulting to Expression.");
		return this.rh.get_EventExpressionInt((CParamExpression)evt.evtParams[paramIndex]);
	}
}
